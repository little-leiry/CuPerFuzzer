...
def main():
    ...
    for name in names:
        ...
        for pl in pls:
            for pg in pgs:
                if pg=='NULL':
                    change_info='    <permission android:name="'+per_name+'" android:protectionLevel="'+pl+'"'+'/>\n'
                    apk_name=name+'-'+pl+'-NULL.apk'
                else:
                    change_info='    <permission android:name="'+per_name+'" android:permissionGroup="android.permission-group.'+pg+'" android:protectionLevel="'+pl+'"/>\n'
                    apk_name=name+'-'+pl+'-'+pg.split('.')[-1]+'.apk'
                num+=1
                print("Build APK:", apk_name)
                try:
                    xml2txt()
                    changeTxt(change_info)
                    txt2xml()
                    pack()
                    sign(apk_name)
