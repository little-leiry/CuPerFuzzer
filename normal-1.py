...
##############################################
# one test
def oneTestNormal1(op_seq,install_apk_comb,device_id,effective_case_num):
    ...
    storeCSV("normal-result-1.csv", store_info)
...
# start fuzzing
def fuzzingNormal1():
    device_id = 'FA7B31A06070'
    print('Start testing...')
    start_time = datetime.now()
    print('Start time:',start_time)
    storeTXT('normal-log-1.txt','\nstart time: '+str(start_time))
    storeCSV('normal-result-1.csv', ['op_sequence', 'op_length', 'install_apk_combination', 'granted_permission', 'click_result', 'seed_model'])
    tested_case_num, effective_case_num = getCaseNum('normal1-case_num.txt')
    while True:
        # select a seed
        print('************************************')
        print('Select a seed...')
        ...
        # get tested cases
        log_file = openFile('normal-log-1.txt')
        ...
        while True:
            start_time = datetime.now()
            re_op = oneTestNormal1(op_seq, install_apk_comb, device_id,effective_case_num)
            if re_op != -1:
            ...
            if re_reset == -1:
                ...
                while True: # repeat until reset successfully
                    ...
                    if re_con == 0: # device does not enter the fastboot mode
                        print('Enter the fastboot mode to reset...')
        ...
        print('Effective cases:', effective_case_num)
        changeCaseNum(tested_case_num,effective_case_num,'normal1-case_num.txt')
        ...
        while True:
            re_log = log('normal-log-1.txt', tested_case_num, seed, op_seq, install_apk_comb, start_time, end_time)
...
def test():
    ...
    for group in groups:
        ...
        #install_apk_comb=['TEST-normal-CALENDAR.apk','TEST-normal-NULL.apk','TEST-dangerous-'+group+'.apk','TEST-dangerous-'+group+'.apk']
        valid_case_num=oneTestNormal1(op_seq, install_apk_comb,device_id,valid_case_num)
        re_reset=reset(device_id)
        if re_reset == -1:
            # the first factory reset failed, wait 2s
            time.sleep(2)
            while True:
                re_con = testConnection(device_id)
    ...
    #time.sleep(1)
    #end_time=datetime.now()
    #log('log-1.txt', seed, op_seq, install_apk_comb, start, end_time)
    #apk=apk_base_path1+'TEST-normal-NULL.apk'
    #install(apk,device_id)
#test()
