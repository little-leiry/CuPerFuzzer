...
# 1-installation, 2-uninstallation, 3-upgrade, 4-reboot
...
def downgrade(device_id):
    ...
    try:
        ...
        subprocess.run(cmd_flash, shell=True, check=True)
        time.sleep(68) # wait for rebooting
        return 0
    except Exception as e:
...
##############################################
# one test
def oneTestUpgrade2(op_seq,install_apk_comb,device_id,effective_case_num):
    ...
    storeCSV("upgrade-result-2.csv", store_info)
...
def fuzzingUpgrade2():
    ...
    storeTXT('upgrade-log-2.txt', '\nstart time: ' + str(start_time))
    storeCSV('upgrade-result-2.csv',
    ...
    tested_case_num, effective_case_num = getCaseNum('upgrade2-case_num.txt')
    while True:
        # select a seed
        print('************************************')
        print('Select a seed...')
        ...
        # get tested cases
        log_file = openFile('upgrade-log-2.txt')
        ...
        while True:
            start_time = datetime.now()
            re_op = oneTestUpgrade2(op_seq, install_apk_comb, device_id, effective_case_num)
            if re_op != -1:
        ...
        print('Effective cases:', effective_case_num)
        changeCaseNum(tested_case_num, effective_case_num, 'upgrade2-case_num.txt')
        ...
        while True:
            re_log = log('upgrade-log-2.txt', tested_case_num, seed, op_seq, install_apk_comb, start_time, end_time)
